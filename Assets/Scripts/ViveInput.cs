    ...
    private void Update()
    {
        ...
        if (m_AttractAction.GetStateDown(m_Pose.inputSource))
        {
            foreach (GameObject note in notes)
            {
                ...
                {
                    //print("left down");
                    note.GetComponent<NoteBehavior>().leftControllerAttraction = true;
                }
                if (m_Pose.inputSource == SteamVR_Input_Sources.RightHand)
                {
                    //print("right down");
                    note.GetComponent<NoteBehavior>().rightControllerAttraction = true;
                }
            }
        }
        if(m_AttractAction.GetStateUp(m_Pose.inputSource))
        {
            foreach (GameObject note in notes)
            {
                ...
                {
                    //print("left up");
                    note.GetComponent<NoteBehavior>().leftControllerAttraction = false;
                }
                if (m_Pose.inputSource == SteamVR_Input_Sources.RightHand)
                {
                    //print("right up");
                    note.GetComponent<NoteBehavior>().rightControllerAttraction = false;
                }
            }
        }
    }
